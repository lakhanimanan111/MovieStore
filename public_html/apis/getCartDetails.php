...
$sql = "select m.movieid, m.title, m.genre, m.description, c.quantity, m.price, (SELECT imageurl FROM movieimages WHERE movieid = m.movieid limit 1) as imageurl, (SELECT stockavailable from inventory where movieid = m.movieid) as stockavailable from cart as c join moviedata as m on c.movieid = m.movieid where c.userid = '$userid'";
